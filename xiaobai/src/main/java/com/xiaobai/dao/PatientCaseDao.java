...
public interface PatientCaseDao {
	...
	@Select({ "<script>", "SELECT COUNT(*) FROM docdemo.patientcase N",
			"WHERE N.uuid LIKE CONCAT('%',#{uuid},'%')", "</script>" })
	int count(Patientcase patientcase);

	@Select({ "<script>", "SELECT * FROM docdemo.patientcase N ",
			"WHERE N.uuid LIKE CONCAT('%',#{uuid},'%')",
